...
class BookingRepository extends BaseRepository
{
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == env('CUSTOMER_ROLE_ID')) {
            ...
            $fromLanguageId = GeneralHelpers::getResponseArrayNotIsset($data['from_language_id'], 'from_language_id');
            if($fromLanguageId != Null){
                return $fromLanguageId;
            }

            if ($data['immediate'] == 'no') {
                $due_date_res = GeneralHelpers::getResponseArrayIsset($data['due_date'], 'due_date', 'Du måste fylla in alla fält');
                if($due_date_res != Null){
                    return $due_date_res;
                }
                $due_date_res = GeneralHelpers::getResponseArrayIsset($data['due_time'], 'due_time', 'Du måste fylla in alla fält');
                if($due_date_res != Null){
                    return $due_date_res;
                }
            ...
            $duration_res = GeneralHelpers::getResponseArrayIsset($data['duration'], 'duration', 'Du måste fylla in alla fält');
            if($duration_res != Null){
                return $duration_res;
            }
            $data['customer_phone_type'] = isset($data['customr_phone_type']) ? 'yes' : 'no';
            $data['customer_physical_type'] = isset($data['customr_physical_type']) ? 'yes' : 'no';
            $response['customr_physical_type'] = $data['customr_physical_type'];
            ...
            $data['gender'] = in_array('male', $data['job_for']) ? 'male' : (in_array('female', $data['job_for'])? 'female' : 'other');

            $data['certified'] = GeneralHelpers::checkArraySingle('normal', 'normal', $data['job_for']);
            $data['certified'] = GeneralHelpers::checkArraySingle('certified', 'yes', $data['job_for']);
            $data['certified'] = GeneralHelpers::checkArraySingle('certified_in_law', 'law', $data['job_for']);
            $data['certified'] = GeneralHelpers::checkArraySingle('certified_in_health', 'health', $data['job_for']);
            $data['certified'] = GeneralHelpers::checkArrayDouble('normal', 'certified', 'both', $data['job_for']);
            $data['certified'] = GeneralHelpers::checkArrayDouble('normal', 'certified_in_law', 'n_law', $data['job_for']);
            $data['certified'] = GeneralHelpers::checkArrayDouble('normal', 'certified_in_health', 'n_health', $data['job_for']);

            $data['job_type'] = GeneralHelpers::getConsumerType($consumer_type);
            ...
            $data['job_for'] = array();
            if ($job->gender != null) {
                $data['job_for'][] = GeneralHelpers::mapGenderToSwedish($job->gender);
            }
            if ($job->certified != null) {
                $data['job_for'][] = GeneralHelpers::getJobFor($job->certified);
            }
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    public function storeJobEmail($data)
    {
        ...
        $email = GeneralHelpers::getUserEmail($job, $user);
        $name = $user->name;
    ...
    public function jobToData($job)
    {
        ...
        $data = [
            ...
            'due_date' => '',
            'due_time' => '',
            'job_for' => GeneralHelpers::getJobForArray($job),
        ];
    ...
    public function sendPushNotificationToSpecificUsers($users, $job_id, $data, $msg_text, $is_need_delay)
    {
        ...
        if ($data['notification_type'] == 'suitable_job') {
            $type_of_sound = ($data['immediate'] == 'no') ? 'normal' : 'emergency';
            $android_sound = $type_of_sound.'_booking';
            $ios_sound = $type_of_sound.'_booking.mp3';
        }
        ...
        $response = GeneralHelpers::getCurlRequest('https://onesignal.com/api/v1/notifications', 'Content-Type: application/json', $onesignalRestAuthKey, $fields);
    ...
    public function getPotentialTranslators(Job $job)
    {
        ...
        $translator_type = GeneralHelpers::getTranslatorType($job_type);

        $joblanguage = $job->from_language_id;
        $gender = $job->gender;
        $translator_level = GeneralHelpers::getTranslatorLevel($job);
...
//        }
    ...
    private function sendNotificationChangePending($user, $job, $language, $due, $duration)
    {
        $data = array();
        $data['notification_type'] = 'session_start_remind';
        $oneWordChange = ($job->customer_physical_type == 'yes') ? 'platstolkningen' : 'telefontolkningen';
        $msg_text = array(
            "en" => 'Du har nu fått '.$oneWordChange.' för ' . $language . ' kl ' . $duration . ' den ' . $due . '. Vänligen säkerställ att du är förberedd för den tiden. Tack!'
        );
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function bookingExpireNoAccepted()
    {
        ...
        if ($cuser && ($cuser->is('superadmin') || $cuser->is('admin'))) {
            ...
            if (isset($requestdata['lang']) && $requestdata['lang'] != '') {
                $allJobs = DBHelpers::addDBWhereIn($allJobs, 'jobs.form_language_id', $requestdata['lang']);
            }
            
            if (isset($requestdata['status']) && $requestdata['status'] != '') {
                $allJobs = DBHelpers::addDBWhereIn($allJobs, 'jobs.status', $requestdata['status']);
            }

            if (isset($requestdata['customer_email']) && $requestdata['customer_email'] != '') {
                $user = DB::table('users')->where('email', $requestdata['customer_email'])->first();
                if ($user) {
                    $allJobs = DBHelpers::addDBWhere($allJobs, 'jobs.user_id', $user->id);
                }
            }
            ...
            if (isset($requestdata['translator_email']) && $requestdata['translator_email'] != '') {
                ...
                if ($user) {
                    $allJobIDs = DB::table('translator_job_rel')->where('user_id', $user->id)->lists('job_id');
                    $allJobs = DBHelpers::addDBWhereIn($allJobs, 'jobs.id', $allJobIDs);
                }
            }
            if (isset($requestdata['filter_timetype']) && $requestdata['filter_timetype'] == "created") {
                if (isset($requestdata['from']) && $requestdata['from'] != "") {
                    $allJobs = DBHelpers::addDBWhere($allJobs, 'jobs.created_at', $requestdata['from'], '>=');
                }
                if (isset($requestdata['to']) && $requestdata['to'] != "") {
                    $to = $requestdata["to"] . " 23:59:00";
                    $allJobs = DBHelpers::addDBWhere($allJobs, 'jobs.created_at', $to, '<=');
                }
                $allJobs->orderBy('jobs.created_at', 'desc');
            }
            if (isset($requestdata['filter_timetype']) && $requestdata['filter_timetype'] == "due") {
                if (isset($requestdata['from']) && $requestdata['from'] != "") {
                    $allJobs = DBHelpers::addDBWhere($allJobs, 'jobs.due', $requestdata['from'], '>=');
                }
                if (isset($requestdata['to']) && $requestdata['to'] != "") {
                    $to = $requestdata["to"] . " 23:59:00";
                    $allJobs = DBHelpers::addDBWhere($allJobs, 'jobs.due', $to, '<=');
                }
                $allJobs->orderBy('jobs.due', 'desc');
            }

            if (isset($requestdata['job_type']) && $requestdata['job_type'] != '') {
                $allJobs = DBHelpers::addDBWhereIn($allJobs, 'jobs.job_type', $requestdata['job_type']);
            }
            $allJobs->select('jobs.*', 'languages.language')
